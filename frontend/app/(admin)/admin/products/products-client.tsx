...
import { fetchAdminProductsAction } from './actions'
...
export function ProductsClient({ initialProducts }: Props) {
  ...
  const load = useCallback(async (term: string) => {
    ...
    setError(null)
    try {
      const result = await fetchAdminProductsAction(term)
      if (!result.ok) throw new Error(result.error)
      const data = result.products
      // #region agent log
      fetch('http://127.0.0.1:7589/ingest/5a232d27-556f-403d-98b2-82415887fe5c',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'8b5927'},body:JSON.stringify({sessionId:'8b5927',runId:'post-fix',hypothesisId:'H5',location:'products-client.tsx:load',message:'admin products loaded',data:{search:term,productCount:data?.length??0},timestamp:Date.now()})}).catch(()=>{});
      // #endregion
      setProducts(data as unknown as AdminProductRow[])
    } catch (err) {
      // #region agent log
      fetch('http://127.0.0.1:7589/ingest/5a232d27-556f-403d-98b2-82415887fe5c',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'8b5927'},body:JSON.stringify({sessionId:'8b5927',runId:'post-fix',hypothesisId:'H1-H5',location:'products-client.tsx:load',message:'admin products load failed',data:{search:term,error:err instanceof Error?err.message:String(err)},timestamp:Date.now()})}).catch(()=>{});
      // #endregion
