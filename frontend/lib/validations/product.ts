...
export const productFormSchema = z.object({
  ...
  slug:              z.string().min(3, 'اسلاگ حداقل ۳ کاراکتر باشد')
                       .regex(/^[\p{L}\p{N}]+(?:-[\p{L}\p{N}]+)*$/u, 'حروف فارسی یا انگلیسی، عدد و خط تیره مجاز است'),
  ...
  stock_left:   z.coerce.number().int().min(0, 'موجودی چپ‌بازشو باید ≥ ۰ باشد').default(0),
  stock_right:  z.coerce.number().int().min(0, 'موجودی راست‌بازشو باید ≥ ۰ باشد').default(0),
  ...
  dimension_unit: z.enum(['cm', 'mm', 'm']).default('cm'),
  dimension_option_1: z.string().max(100).optional().or(z.literal('')),
  dimension_option_2: z.string().max(100).optional().or(z.literal('')),
  dimension_option_3: z.string().max(100).optional().or(z.literal('')),
  allow_custom_dimensions: z.boolean().default(false),
